(() => {
  const app = {
    ...
    async fetchArtApi () {
      // call function ArtApi from service.json 
    ...
    updateArtUi () {
      ...
      // Get the first value associated with the given search parameter
      const params = new URLSearchParams(search);
      const urlCategory = params.get('category');

      // Do soomtehing if urlCategory contains a category
      if (urlCategory !== null) {
        // Filter all the articles with the same category and return all the articles with the same category as urlCategopry
        ...
        // filter all articles with the same category based on their year
        let tempStr = '';
    ...
    getHTMLForYearsList () {
       // Return the querystring part of a URL (catezgory) 
       // Set quesrystrinas part of the a tag so if on a category you''l be able to scroll to the right year
      const search = window.location.search;
    ...
    activeCategory () {
      ...
      // loop through the list of all categories 
      for (let i = 0; i < this.$categoriesListLink.length; i++) {
        //if the category in the list has the same url in the a-tag as the url on the webpage 
        // then add classname active
